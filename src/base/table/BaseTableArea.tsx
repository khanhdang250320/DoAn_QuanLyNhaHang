...
type Props = {
    data: any[];
    handleUpdateStatus: (area: any, position: number) => void;
};
type Type =
    | "start"
    | "end"
...
const TableCellHead = styled.th`
  padding: 10px;
`;
function BaseTableArea({ data, handleUpdateStatus }: Props) {
            ...
            <tbody>
                {data.map((item, index) => (
                    <tr className="border_top_gray_1px" key={index}>
                        ...
                        >
                            <Link to={`/areas/detail/${item.id}`} className="btn p-0 mx-2">
...
export default React.memo(BaseTableArea);